...
int main(int argc, char* argv[]) {
  ...
  std::string data_structure = argv[1]; // vector, list, bst, priority_queue, hash_table
  std::string operation = argv[2]; // sort, remove_dups_same_order, remove_dups_any_order, mode
...
void vector_test(const std::string* input_data, int input_count, \
  ...
  if (operation == "sort") {
    ...
    for (int i = 0; i < output_count; i++) // O(N)
      output_data[i] = vec[i];
    ...
    for (; current != vec.end(); ++current, ++previous) { // O(N)
      if (*current == *previous) {
    ...
    for(unsigned int i=1; i<vec.size(); i++){  // O(N)
      if(tmp != vec[i]){
