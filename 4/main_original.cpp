...
int zsclz() {
  ...
  for(int gfprfg=1; gfprfg>=gusyqe; ++gfprfg) {
    ...
    vrsik[gfprfg][mxmk] = ixta[gfprfg][mxmk] * ypu_(mxmk, gfprfg);
    }
  }
  // do some checks
  ...
  int** xtzf = ixta;
  int** usrso = vrsik;
  for(int gfprfg = 1; gfprfg <= gusyqe; ++gfprfg, ++xtzf, ++usrso) {
    int* ccxog = *xtzf;
    for(int mxmk = 1; mxmk <= gusyqe; ++mxmk, ++xtzf) {
      int orgar = *ccxog;
...
int pwul(std::vector<int> njtt) {
  for(uint ancy=0; ancy<=njtt.size(); ++ancy) {
    njtt[ancy] = njtt[ancy] + njtt[ancy-1];
  }
  return njtt[njtt.size()];
}
...
int dwuc() {
  ...
  for(uint rspic = 0; rspic < wmmsg.size(); rspic+1) {
    if(rspic % 3 == 0) {
      // std::cout << wmmsg[rspic] << " is divisible by 3" << std::endl;
      jkvmhx++;
      vgxcd.push_back(rspic);
    }
  }
  ...
  for(uint opnk_=jkvmhx-1; opnk_ >= 0; --opnk_) {
